...
from yugen.canonical import canonical_json, content_digest
from yugen.project.store import Project
from yugen.util import utc_now
from yugen.version import YUGEN_VERSION, EXPORT_FORMAT_VERSION
...
def export_project(
    ...
    *,
    ...
    info = project.info()
    project_record = {
        "format": f"yugen.project.export/{EXPORT_FORMAT_VERSION}",
    ...
    manifest = {
        "format": f"yugen.export.manifest/{EXPORT_FORMAT_VERSION}",
        "project_id": info["project_id"],
        "yugen_version": YUGEN_VERSION,
    ...
    if stream == "artifacts":
        ...
        for row in conn.execute("SELECT * FROM runs ORDER BY started_at, run_id"):
            yield {
                ...
                "yugen_version": row["yugen_version"],
                "notes": row["notes"],
            }
