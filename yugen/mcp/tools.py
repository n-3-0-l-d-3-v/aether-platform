...
from yugen.errors import YugenError, EvidenceError
    ...
    if not results:
        return {
            ...
            "hint": (
                ...
                "from a Ghidra run; check yugen_project_info for whether the "
...
def _ask(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    ...
    from yugen.nl import ask as run_ask
...
def _diff_graph(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    from yugen.export.diff import diff_snapshots, load_snapshot, snapshot_from_project
...
def _campaign(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    ...
    from yugen.project.store import Project as ProjectClass

    other_paths = list(args.get("other_projects") or [])
    if not other_paths:
        raise EvidenceError(
            "yugen_campaign compares this project against at least one other "
...
def _submit_claim(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not evidence_args:
        raise EvidenceError(
            ...
            "one artifact id; find them with yugen_find_artifacts or "
            "yugen_search_strings first."
        )
        ...
        _schema(
            {
                "kind": {
                    "type": "string",
                    "description": "Artifact kind. Use yugen_describe_schema to list them.",
                },
...
_register(
    Tool(
        ...
        "before calling yugen_submit_claim.",
        _schema(
            {
...
_register(
    Tool(
        ...
        "directly with yugen_find_claims.",
        _schema(
            {
...
_register(
    Tool(
        ...
        "running 'yugen review approve/reject' at the CLI, can promote a "
...
_register(
    Tool(
        ...
        "that import resolved to another file's export (from yugen_map). The "
...
def call_tool(
    ...
    """Dispatch a tool call, translating Yugen errors into useful messages."""
    tool = TOOLS.get(name)
    if tool is None:
        raise EvidenceError(
