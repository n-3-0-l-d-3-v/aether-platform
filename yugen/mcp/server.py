...
Yugen is local-first and its CLI has zero runtime dependencies; making the MCP
server the one component that drags in an async framework and a web stack would
undercut that. The protocol surface Yugen needs - initialize, tools/list,
...
:mod:`yugen.mcp.tools`. Swapping this file for the official SDK later means
...
from yugen.errors import YugenError, EvidenceError, SchemaError
...
class MCPServer:
    ...
    def handle_message(self, message: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            if method in ("resources/list", "resources/templates/list"):
                # Yugen exposes everything through tools; answering these
    ...
    def _initialize(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "serverInfo": {"name": SERVER_NAME, "version": YUGEN_VERSION},
            "instructions": (
                f"Yugen evidence graph for project '{info['name']}'. "
                "Start with yugen_project_info, then yugen_list_objects. "
                "Every finding is a claim backed by artifact ids: use "
                "yugen_find_claims to read them and yugen_get_claim to see "
                "what each one rests on. If you conclude something yourself, "
                "record it with yugen_submit_claim - read "
                "yugen_describe_schema first, because statements must match a "
                "registered predicate and cite real artifacts. Free-text "
                "findings are rejected by design; use yugen_annotate for "
                "commentary."
            ),
        }
