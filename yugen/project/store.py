...
:mod:`yugen.evidence.schemas` from being one convenient shortcut away from
being bypassed.
...
from yugen.canonical import canonical_json, mint_id
...
from yugen.project import db
from yugen.util import utc_now
from yugen.version import YUGEN_VERSION, SCHEMA_VERSION
...
class Project:
    """An Yugen project directory backed by SQLite."""
    ...
    def create(root: str, name: str | None = None, *, exist_ok: bool = False) -> "Project":
        ...
        project_name = name or os.path.basename(root) or "yugen-project"
        if db.current_version(conn) and not project.meta("project_id"):
            ...
            project._set_meta("yugen_version", YUGEN_VERSION)
        return project
    ...
    def open(root: str, *, read_only: bool = False) -> "Project":
        ...
        if not os.path.exists(db_path):
            raise ProjectError(
                f"no Yugen project at {root} (expected {db.DB_FILENAME}); "
                "run 'yugen init' first"
            )
    ...
    def run(
        ...
        *,
        ...
        self._conn.execute(
            ...
            " started_at, status, yugen_version, notes)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                run_id,
                tool,
        ...
        try:
            ...
            self._conn.execute(
                ...
                " input_digest, started_at, finished_at, status, yugen_version, notes)"
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    run_id,
                    tool,
    ...
    def set_claim_status(self, claim_id: str, status: str) -> None:
        ...
        from yugen.evidence.models import CLAIM_STATUSES
