...
Together with the checks in :mod:`yugen.project.store`, that makes "every
...
from yugen.errors import ProjectError
from yugen.version import SCHEMA_VERSION

#: Filename of the project database inside the project directory.
DB_FILENAME = "yugen.db"
...
CREATE TABLE runs (
    ...
    exit_code      INTEGER,
    yugen_version TEXT NOT NULL DEFAULT '',
...
def connect(db_path: str, *, read_only: bool = False) -> sqlite3.Connection:
    """Open a project database with Yugen's standard pragmas."""
    if read_only:
...
def migrate(conn: sqlite3.Connection) -> int:
    """Bring the database up to :data:`yugen.version.SCHEMA_VERSION`."""
    version = current_version(conn)
    if version > SCHEMA_VERSION:
        raise ProjectError(
            f"project was written by a newer Yugen (schema v{version}); "
...
def integrity_problems(conn: sqlite3.Connection) -> list[dict[str, object]]:
    ...
    Used by ``yugen check``. An empty list is the invariant holding; anything
