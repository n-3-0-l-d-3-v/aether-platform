...
This module is the enforcement point for Yugen's first absolute principle:
...
any predicate - if a producer wants to say something Yugen cannot represent,
...
``ARTIFACT_KINDS``
    ...
    :func:`yugen.canonical.mint_id`, so they decide when two observations are
...
``CLAIM_PREDICATES``
    ...
    advisory: :mod:`yugen.project.store` refuses the insert.
"""
...
from yugen.canonical import is_id
from yugen.errors import SchemaError
...
class Field:
    ...
    def validate(self, value: Any, where: str) -> Any:
        ...
        if self.type == "id":
            if not is_id(value):
                raise SchemaError(
                    f"{where}.{self.name} must be a Yugen id, got {value!r}"
                )
            return value
...
#: Formats Yugen normalizes to. Kept short on purpose: this is triage, not
    ...
    for k in (
        ...
        _kind(
            ...
            Field(
                ...
                "str",
                required=True,
                doc="Which tool reported the hit, e.g. binwalk or yugen-carver.",
            ),
            Field("size", "int", minimum=0),
        ),
    )
...
class ClaimPredicate:
    ...
    def schema_id(self) -> str:
        return f"yugen.claim.{self.name}/{self.revision}"
...
def check_evidence_requirements(
    ...
    Raises :class:`yugen.errors.EvidenceError` when a requirement is unmet -
    this is the check that makes "claims must be backed by evidence" real.
    """
    from yugen.errors import EvidenceError  # local import: avoids a cycle
