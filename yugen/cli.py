...
from yugen.errors import YugenError, ProjectError
from yugen.project.store import Project
from yugen.util import hex_addr, human_size, sanitize_text
...
def _open_project(args: argparse.Namespace, *, read_only: bool = False) -> Project:
    ...
    if not root:
        raise ProjectError(
            "no Yugen project found here or in any parent directory. "
            "Run 'yugen init' first, or pass --project <dir>."
        )
...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    from yugen.adapters.triage.formats import identify_file
        ...
        if engine == "firmware":
            ...
            if engine in ("binary", "ghidra"):
                ...
                if availability.available:
                    ...
                    raise YugenError(f"{availability.detail}\n  {availability.remedy}")
                else:
                    _warn(
...
def cmd_query(args: argparse.Namespace) -> int:
    # The schema is a property of Yugen, not of any project. Requiring one
...
def _query_schema(args: argparse.Namespace) -> int:
    from yugen.evidence.schemas import describe_registries
    ...
    if args.name:
        for bucket in ("claim_predicates", "artifact_kinds"):
            if args.name in registries[bucket]:
                ...
                return 0
        raise YugenError(f"unknown predicate or artifact kind {args.name!r}")
...
def _dispatch_query(project: Project, args: argparse.Namespace) -> int:
    ...
    if what == "claim":
        ...
        if claim is None:
            raise YugenError(f"unknown claim {args.id}")
        evidence = []
    ...
    raise YugenError(f"unknown query target {what!r}")
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    from yugen.adapters.binwalk import BinwalkAdapter
    from yugen.adapters.ghidra import GhidraAdapter, probe_java
    ...
    def render(rows: dict[str, Any]) -> None:
        print(f"yugen {YUGEN_VERSION}  (python {sys.version.split()[0]}, {sys.platform})")
        print()
        ...
        if available < len(rows):
            print(
                "Yugen still runs: header triage, firmware carving, the evidence "
...
def cmd_ask(args: argparse.Namespace) -> int:
    ...
    from yugen.nl import ask, describe_supported
    ...
    if not args.question:
        raise YugenError(
            "ask what? Pass a question, or 'yugen ask --list' to see what "
...
def cmd_diff_graph(args: argparse.Namespace) -> int:
    ...
    """
    from yugen.export.diff import load_snapshot, diff_snapshots
...
def cmd_campaign(args: argparse.Namespace) -> int:
    ...
    if len(args.projects) < 2:
        raise YugenError("need at least two projects to correlate")
...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    MCP tools, so an agent can see the queue (via yugen_review_queue) but can
    ...
    try:
        ...
        if not args.claim_id:
            raise YugenError("review approve/reject needs a claim id")
...
def cmd_mcp(args: argparse.Namespace) -> int:
    ...
    root = args.project or Project.discover()
    if not root:
        raise YugenError("no Yugen project found; run 'yugen init' first")
    return serve_project(root, read_only=args.read_only)


def cmd_eval(args: argparse.Namespace) -> int:
    from yugen.eval import run_suites

    paths = args.suites or _default_suites()
    if not paths:
        raise YugenError("no suites given and none found under eval/suites/")
...
def _resolve_object_id(project: Project, reference: str | None) -> str | None:
    ...
    if artifact is None:
        raise YugenError(f"no file in this project matches {reference!r}")
...
def _parse_addr(value: str | None) -> int | None:
    ...
    try:
        ...
        raise YugenError(f"could not parse address {value!r}") from exc
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"yugen {YUGEN_VERSION}")
    parser.add_argument(
        "--project",
        "-P",
    ...
    p_reach = subparsers.add_parser(
        ...
        description=(
            ...
            "results imported (call-site xrefs), and 'yugen map' already run "
