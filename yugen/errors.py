...
class YugenError(Exception):
    """Base class for every error Yugen raises deliberately."""

    exit_code = 1


class ProjectError(YugenError):
    """Project could not be created, opened, or migrated."""

    exit_code = 2


class SchemaError(YugenError):
...
class EvidenceError(YugenError):
