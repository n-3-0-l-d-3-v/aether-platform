# Export a Ghidra program to Yugen's deterministic JSONL interchange format.
#
# Runs inside Ghidra headless (analyzeHeadless -postScript YugenExport.py ...).
...
#
# Everything is written sorted - by address, then by name - because Yugen's
# export is meant to diff cleanly in Git, and a re-analysis that discovered
# nothing new should produce no diff at all.
#
# Arguments (all optional, positional):
#   [0] output directory                (default: program-name.yugen next to cwd)
...
EXPORT_FORMAT = "yugen.ghidra.export/1"
DEFAULT_DECOMPILE_LIMIT = 40
DECOMPILE_TIMEOUT_SECONDS = 60

# Reference types Yugen models. Anything else collapses to "unknown" rather
...
def rank_functions(functions, program, interesting_names):
    ...
    Yugen, so the policy stays on Yugen's side of the bridge.
    """
    wanted = set()
...
def main():
    ...
    out_dir = args[0] if len(args) > 0 else (program.getName() + ".yugen")
    try:
    ...
    print("[yugen] exported to {0}".format(out_dir))
    for stream in sorted(counts.keys()):
        print("[yugen]   {0}: {1}".format(stream, counts[stream]))
