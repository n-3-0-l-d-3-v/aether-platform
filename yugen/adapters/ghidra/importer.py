...
from yugen.adapters.triage import detectors
...
from yugen.util import sanitize_text
...
def read_export(export_dir: str) -> dict[str, Any]:
    ...
    if not os.path.isfile(meta_path):
        raise AdapterError(
            f"{export_dir} does not look like a Yugen/Ghidra export "
            "(no meta.json). Was YugenExport.py given the right output path?"
        )
