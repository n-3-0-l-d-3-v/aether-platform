...
jffs2, ubifs, and a hundred vendor formats that Yugen has no business
reimplementing. Falls back to :mod:`yugen.adapters.binwalk.carver` otherwise,
...
from yugen.adapters.binwalk import carver
...
from yugen.util import logical_path as normalize_path
...
class BinwalkAdapter(Adapter):
    ...
    def engine(self) -> str:
        ...
        return "binwalk" if (self.prefer_binwalk and self._binary) else "yugen-carver"
    ...
    def analyze(
        ...
        *,
        ...
        with project.run(
            tool=engine,
            tool_version=self.probe().version if engine == "binwalk" else YUGEN_VERSION,
            adapter=self.name,
