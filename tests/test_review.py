    ...
    response = server.handle_message(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "aether_review_queue", "arguments": {}},
        }
    )["result"]
...
def test_no_mcp_tool_can_approve_or_reject_a_claim(project):
    ...
    # aether_submit_claim exists and writes proposals; nothing promotes one.
    assert "aether_submit_claim" in names
    assert tools.TOOLS["aether_review_queue"].writes is False
