...
from ultron.mcp.server import PROTOCOL_VERSION, MCPServer
...
def test_initialize_advertises_tools_and_guidance(server):
    ...
    assert result["serverInfo"]["name"] == "ultron"
    assert "ultron_submit_claim" in result["instructions"]
...
def test_project_info_reports_engine_availability(server):
    payload = call(server, "ultron_project_info")["structuredContent"]
...
def test_list_and_get_object(server):
    listed = call(server, "ultron_list_objects")["structuredContent"]
    assert listed["objects"][0]["format"] == "elf"

    detail = call(server, "ultron_get_object", {"object": "firmware_agent"})[
        "structuredContent"
    ]
...
def test_search_strings_returns_locations(server):
    payload = call(server, "ultron_search_strings", {"query": "AKIA"})["structuredContent"]
...
def test_claims_resolve_to_their_evidence(server):
    found = call(server, "ultron_find_claims", {"predicate": "contains_hardcoded_secret"})
    claim_id = found["structuredContent"]["claims"][0]["claim_id"]

    detail = call(server, "ultron_get_claim", {"claim_id": claim_id})["structuredContent"]
...
def test_describe_schema_documents_evidence_requirements(server):
    payload = call(
        server, "ultron_describe_schema", {"predicate": "contains_hardcoded_secret"}
    )["structuredContent"]
...
def test_neighbors_walks_from_a_claim(server):
    found = call(server, "ultron_find_claims", {"limit": 1})["structuredContent"]
    claim_id = found["claims"][0]["claim_id"]
    graph = call(server, "ultron_neighbors", {"node_id": claim_id, "depth": 2})[
        "structuredContent"
    ]
    assert graph["nodes"]
    assert graph["edges"]
...
def test_decompilation_absent_gives_an_actionable_hint(server):
    payload = call(server, "ultron_get_decompilation", {"function": "main"})[
        "structuredContent"
    ]
...
def test_unknown_object_reference_is_a_tool_error_not_a_crash(server):
    result = call(server, "ultron_get_object", {"object": "no-such-file"})
    assert result["isError"] is True
    assert "no file in this project" in result["content"][0]["text"]


def test_unknown_tool_lists_the_available_ones(server):
    result = call(server, "ultron_do_my_job")
    assert result["isError"] is True
    assert "ultron_find_claims" in result["content"][0]["text"]
...
def test_read_only_servers_hide_and_refuse_write_tools(project, elf_sample):
    ...
    assert "ultron_submit_claim" not in names
    assert "ultron_find_claims" in names

    result = call(readonly, "ultron_submit_claim", {"predicate": "x", "statement": {},
                                                    "evidence": [{"artifact_id": "a"}]})
    assert result["isError"] is True
    assert "read-only" in result["content"][0]["text"]


def test_ultron_map_resolves_cross_binary_links(project):
    ...
    result = call(server, "ultron_map")["structuredContent"]
    assert result["links_found"] == 1
    assert result["graph"]["edges"][0]["symbol"] == "EVP_EncryptUpdate"


def test_ultron_map_is_hidden_and_refused_read_only(project):
    with project.run(tool="t", tool_version="1", adapter="test") as rc:
        rc.artifact(
    ...
    readonly = MCPServer(project, read_only=True)
    names = {t["name"] for t in request(readonly, "tools/list")["result"]["tools"]}
    assert "ultron_map" not in names
    result = call(readonly, "ultron_map")
    assert result["isError"] is True
    assert "read-only" in result["content"][0]["text"]


def test_ultron_diff_graph_compares_against_a_baseline_on_disk(project, tmp_path):
    ...
    server = MCPServer(project)
    result = call(server, "ultron_diff_graph", {"baseline_path": baseline_dir})["structuredContent"]
    assert result["summary"]["artifacts_added"] == 1
    assert not result["identical"]


def test_ultron_diff_graph_reports_identical_when_nothing_changed(project, tmp_path):
    baseline_dir = str(tmp_path / "baseline")
    baseline = Project.create(baseline_dir, "baseline")
    with baseline.run(tool="t", tool_version="1", adapter="test") as rc:
        rc.artifact(
