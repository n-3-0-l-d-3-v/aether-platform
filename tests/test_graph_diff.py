...
from ultron.export.diff import (
    DiffError,
    diff_paths,
    diff_snapshots,
    load_snapshot,
...
def test_an_unrecognisable_path_is_a_clean_error(tmp_path):
    ...
    empty.mkdir()
    with pytest.raises(DiffError, match="neither a Ultron project"):
...
def test_an_export_directory_missing_the_graph_stream_is_a_clean_error(tmp_path):
    ...
    fake_export.mkdir()
    with pytest.raises(DiffError, match="does not look like a Ultron export"):
...
def test_cli_diff_graph_on_a_bad_path_is_a_clean_error(tmp_path, capsys):
    ...
    assert "neither a Ultron project" in capsys.readouterr().err
