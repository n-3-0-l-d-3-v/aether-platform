...
from ultron.adapters.binwalk import BinwalkAdapter, carver
from ultron.adapters.ghidra import GhidraAdapter, importer
from ultron.adapters.triage import TriageAdapter, detectors, formats, strings
...
def test_export_script_is_shipped_with_the_package():
    from ultron.adapters.ghidra import scripts_dir

    script = os.path.join(scripts_dir(), "UltronExport.py")
...
def test_headless_invocation_is_well_formed(tmp_path):
    ...
    from ultron.adapters.ghidra import ANALYSIS_TIMEOUT_SECONDS, GHIDRA_PROJECT_NAME
    ...
    assert argv[script_index + 1] == "UltronExport.py"
...
def test_suspicious_claims_are_capped_per_category(project, tmp_path):
    ...
    from ultron.adapters.triage import MAX_SUSPICIOUS_PER_CATEGORY, TriageAdapter
