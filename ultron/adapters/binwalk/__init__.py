...
jffs2, ubifs, and a hundred vendor formats that Ultron has no business
reimplementing. Falls back to :mod:`ultron.adapters.binwalk.carver` otherwise,
...
from ultron.adapters.binwalk import carver
...
from ultron.util import logical_path as normalize_path
...
class BinwalkAdapter(Adapter):
    ...
    def engine(self) -> str:
        ...
        return "binwalk" if (self.prefer_binwalk and self._binary) else "ultron-carver"
    ...
    def analyze(
        ...
        *,
        ...
        with project.run(
            tool=engine,
            tool_version=self.probe().version if engine == "binwalk" else ULTRON_VERSION,
            adapter=self.name,
