...
from ultron.adapters.triage import detectors
...
from ultron.util import sanitize_text
...
def read_export(export_dir: str) -> dict[str, Any]:
    ...
    if not os.path.isfile(meta_path):
        raise AdapterError(
            f"{export_dir} does not look like a Ultron/Ghidra export "
            "(no meta.json). Was UltronExport.py given the right output path?"
        )
