...
from ultron.canonical import canonical_json, content_digest
from ultron.project.store import Project
from ultron.util import utc_now
from ultron.version import ULTRON_VERSION, EXPORT_FORMAT_VERSION
...
def export_project(
    ...
    *,
    ...
    info = project.info()
    project_record = {
        "format": f"ultron.project.export/{EXPORT_FORMAT_VERSION}",
    ...
    manifest = {
        "format": f"ultron.export.manifest/{EXPORT_FORMAT_VERSION}",
        "project_id": info["project_id"],
        "ultron_version": ULTRON_VERSION,
    ...
    if stream == "artifacts":
        ...
        for row in conn.execute("SELECT * FROM runs ORDER BY started_at, run_id"):
            yield {
                ...
                "ultron_version": row["ultron_version"],
                "notes": row["notes"],
            }
