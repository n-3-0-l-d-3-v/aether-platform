...
Supplements :mod:`ultron.adapters.triage.detectors` - the deterministic,
...
1. **Never bypasses human review.** Every claim this agent proposes is
   ...
   ``status="proposed"`` (see :mod:`ultron.project.store` and
   :mod:`ultron.review`) - identical to any other agent-attested claim. There
   is no path from here to ``accepted`` that does not go through a human
   running ``ultron review approve``. See ADR 0009 for why that boundary is
...
2. **Local-only backend is a hard boundary, not a default.** The caller must
   construct and pass an :class:`~ultron.agents.backend.LLMBackend`; nothing
   here reaches out to a cloud API, and nothing here silently falls back to
   one. See :mod:`ultron.agents.backend`.
...
``ultron agent secrets`` (or calls :func:`run_secrets_triage` directly). Nothing
in ``ultron analyze`` or any adapter calls it implicitly.
"""
