...
class ProjectError(UltronError):
    """Project could not be created, opened, or migrated."""

    exit_code = 2


class SchemaError(UltronError):
...
class EvidenceError(UltronError):
...
class AdapterError(UltronError):
