...
Ultron is local-first and its CLI has zero runtime dependencies; making the MCP
server the one component that drags in an async framework and a web stack would
undercut that. The protocol surface Ultron needs - initialize, tools/list,
...
:mod:`ultron.mcp.tools`. Swapping this file for the official SDK later means
...
from ultron.errors import UltronError, EvidenceError, SchemaError
...
class MCPServer:
    ...
    def handle_message(self, message: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            if method in ("resources/list", "resources/templates/list"):
                # Ultron exposes everything through tools; answering these
    ...
    def _initialize(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "serverInfo": {"name": SERVER_NAME, "version": ULTRON_VERSION},
            "instructions": (
                f"Ultron evidence graph for project '{info['name']}'. "
                "Start with ultron_project_info, then ultron_list_objects. "
                "Every finding is a claim backed by artifact ids: use "
                "ultron_find_claims to read them and ultron_get_claim to see "
                "what each one rests on. If you conclude something yourself, "
                "record it with ultron_submit_claim - read "
                "ultron_describe_schema first, because statements must match a "
                "registered predicate and cite real artifacts. Free-text "
                "findings are rejected by design; use ultron_annotate for "
                "commentary."
            ),
        }
...
def serve_project(project_root: str, *, read_only: bool = False) -> int:
    """Entry point used by ``ultron mcp``."""
