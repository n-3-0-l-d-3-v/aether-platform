...
from ultron.errors import UltronError, EvidenceError
    ...
    if not results:
        return {
            ...
            "hint": (
                ...
                "from a Ghidra run; check ultron_project_info for whether the "
...
def _ask(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    ...
    from ultron.nl import ask as run_ask
...
def _diff_graph(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    from ultron.export.diff import diff_snapshots, load_snapshot, snapshot_from_project
...
def _campaign(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    ...
    from ultron.project.store import Project as ProjectClass

    other_paths = list(args.get("other_projects") or [])
    if not other_paths:
        raise EvidenceError(
            "ultron_campaign compares this project against at least one other "
...
def _submit_claim(project: Project, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not evidence_args:
        raise EvidenceError(
            ...
            "one artifact id; find them with ultron_find_artifacts or "
            "ultron_search_strings first."
        )
        ...
        _schema(
            {
                "kind": {
                    "type": "string",
                    "description": "Artifact kind. Use ultron_describe_schema to list them.",
                },
...
_register(
    Tool(
        ...
        "before calling ultron_submit_claim.",
        _schema(
            {
...
_register(
    Tool(
        ...
        "directly with ultron_find_claims.",
        _schema(
            {
...
_register(
    Tool(
        ...
        "running 'ultron review approve/reject' at the CLI, can promote a "
...
_register(
    Tool(
        ...
        "that import resolved to another file's export (from ultron_map). The "
...
def call_tool(
    ...
    """Dispatch a tool call, translating Ultron errors into useful messages."""
    tool = TOOLS.get(name)
    if tool is None:
        raise EvidenceError(
