...
This module is the enforcement point for Ultron's first absolute principle:
...
any predicate - if a producer wants to say something Ultron cannot represent,
...
``ARTIFACT_KINDS``
    ...
    :func:`ultron.canonical.mint_id`, so they decide when two observations are
...
``CLAIM_PREDICATES``
    ...
    advisory: :mod:`ultron.project.store` refuses the insert.
"""
...
from ultron.canonical import is_id
from ultron.errors import SchemaError
...
class Field:
    ...
    def validate(self, value: Any, where: str) -> Any:
        ...
        if self.type == "id":
            if not is_id(value):
                raise SchemaError(
                    f"{where}.{self.name} must be a Ultron id, got {value!r}"
                )
            return value
...
#: Formats Ultron normalizes to. Kept short on purpose: this is triage, not
    ...
    for k in (
        ...
        _kind(
            ...
            Field(
                ...
                "str",
                required=True,
                doc="Which tool reported the hit, e.g. binwalk or ultron-carver.",
            ),
            Field("size", "int", minimum=0),
        ),
    )
...
class ClaimPredicate:
    ...
    def schema_id(self) -> str:
        return f"ultron.claim.{self.name}/{self.revision}"
...
def check_evidence_requirements(
    ...
    Raises :class:`ultron.errors.EvidenceError` when a requirement is unmet -
    this is the check that makes "claims must be backed by evidence" real.
    """
    from ultron.errors import EvidenceError  # local import: avoids a cycle
