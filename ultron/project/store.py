...
:mod:`ultron.evidence.schemas` from being one convenient shortcut away from
being bypassed.
...
from ultron.canonical import canonical_json, mint_id
from ultron.errors import EvidenceError, ProjectError
from ultron.evidence.models import (
    Artifact,
    Attestation,
    Claim,
    ClaimLink,
...
from ultron.project import db
from ultron.util import utc_now
from ultron.version import ULTRON_VERSION, SCHEMA_VERSION
...
class Project:
    """An Ultron project directory backed by SQLite."""
    ...
    def create(root: str, name: str | None = None, *, exist_ok: bool = False) -> "Project":
        ...
        project_name = name or os.path.basename(root) or "ultron-project"
        if db.current_version(conn) and not project.meta("project_id"):
            ...
            project._set_meta("ultron_version", ULTRON_VERSION)
        return project
    ...
    def open(root: str, *, read_only: bool = False) -> "Project":
        ...
        if not os.path.exists(db_path):
            raise ProjectError(
                f"no Ultron project at {root} (expected {db.DB_FILENAME}); "
                "run 'ultron init' first"
            )
    ...
    def run(
        ...
        *,
        ...
        self._conn.execute(
            ...
            " started_at, status, ultron_version, notes)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                run_id,
                tool,
        ...
        try:
            ...
            self._conn.execute(
                ...
                " input_digest, started_at, finished_at, status, ultron_version, notes)"
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    run_id,
                    tool,
    ...
    def set_claim_status(self, claim_id: str, status: str) -> None:
        ...
        from ultron.evidence.models import CLAIM_STATUSES
