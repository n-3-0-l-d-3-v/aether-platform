...
Together with the checks in :mod:`ultron.project.store`, that makes "every
...
from ultron.errors import ProjectError
from ultron.version import SCHEMA_VERSION

#: Filename of the project database inside the project directory.
DB_FILENAME = "ultron.db"
...
CREATE TABLE runs (
    ...
    exit_code      INTEGER,
    ultron_version TEXT NOT NULL DEFAULT '',
...
def connect(db_path: str, *, read_only: bool = False) -> sqlite3.Connection:
    """Open a project database with Ultron's standard pragmas."""
    if read_only:
...
def migrate(conn: sqlite3.Connection) -> int:
    """Bring the database up to :data:`ultron.version.SCHEMA_VERSION`."""
    version = current_version(conn)
    if version > SCHEMA_VERSION:
        raise ProjectError(
            f"project was written by a newer Ultron (schema v{version}); "
...
def integrity_problems(conn: sqlite3.Connection) -> list[dict[str, object]]:
    ...
    Used by ``ultron check``. An empty list is the invariant holding; anything
