...
`proposed`, attested with `producer_kind="agent"` (`yugen_submit_claim`,
Phase 0). What was missing was a queue - a way to see what is waiting - and a
recorded act of a human deciding on it.

## Decision

`yugen/review/` adds exactly that: `pending()` to list proposed claims (by
...
The queue is exposed to MCP as `yugen_review_queue` - read-only. **Approving
and rejecting are CLI-only** (`yugen review approve/reject`) and are not, and
must never become, MCP tools.

## Rationale

An MCP tool is something an agent calls. If `yugen_review_approve` existed
next to `yugen_submit_claim`, an agent - or a chain of agents, or the same
...
`yugen review approve` in a loop. What it removes is the *protocol-level*
...
- Distinguish "this reviewer is authorized to approve claims of this kind"
  ...
  the same trust boundary every other write in Yugen already has.
...
- `yugen_submit_claim` and `yugen_review_queue` together let an agent see
...
- The annotation body (`"accepted by alice: looks legit"`) is free text, and
  ...
  binary, and annotations are exactly where Yugen's evidence model already
