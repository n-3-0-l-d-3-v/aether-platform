...
def trace_cross_binary_reachability(project: Project) -> ReachabilityResult:
    ...
    if not links:
        warnings.append(
            "no imports_resolved_by claims in this project; run 'aether map' "
    ...
    with project.run(
        tool="aether-cartography", tool_version=AETHER_VERSION, adapter="cartography-reach"
    ) as rc:
        sinks_found = 0
        considered = 0
