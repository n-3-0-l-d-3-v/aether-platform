...
def snapshot_from_export(export_dir: str, *, label: str | None = None) -> GraphSnapshot:
    ...
    if not os.path.isfile(artifacts_path) or not os.path.isfile(claims_path):
        raise DiffError(
            f"{export_dir} does not look like an Aether export (expected "
            "graph/artifacts.jsonl and graph/claims.jsonl). Run 'aether export' "
...
def load_snapshot(path: str, *, label: str | None = None) -> GraphSnapshot:
    ...
    A project directory has ``aether.db`` at its root; an export has
    ``graph/artifacts.jsonl``. Both are read-only operations - a diff never
    writes to either side.
    """
    if os.path.isfile(os.path.join(path, "aether.db")):
    ...
    raise DiffError(
        f"{path} is neither an Aether project (no aether.db) nor an export "
